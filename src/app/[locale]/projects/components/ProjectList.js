...
export default function ProjectList({ projects }) {
  ...
  // Hover durumunu ve gecikmeli "hide" çağrısını kontrol etmek için
  ...
  const showPreview = (project) => {
    ...
    img.className =
      'absolute top-0 left-0 object-cover w-full h-full origin-center rounded-lg shadow-2xl';
  ...
  const handleMouseEnter = (e, project) => {
    ...
    // Eğer az önce "hide" planlandıysa iptal et
    if (hideCallRef.current) {
      hideCallRef.current.kill();
      hideCallRef.current = null;
    }
  ...
  const handleMouseLeave = (e) => {
    ...
    // Hızlı A->B hover geçişlerinde yeni resmi yanlışlıkla silmesin diye minicik gecikme
            ...
            <div className="project-roll-wrapper h-[300px] w-full relative">
              ...
              <div className="h-[100px] flex items-center justify-between px-4 md:px-6 bg-black text-white">
                ...
                <div className="flex flex-col text-right">
                  <span className="font-mono text-[10px] md:text-xs uppercase tracking-widest text-[#e3e3db]">
                    {project.client}
                  </span>
              ...
              <div className="h-[100px] flex items-center justify-between px-2 md:px-4 bg-[#f3f2ed] text-black">
                <span className="text-2xl font-bold tracking-tighter uppercase md:text-5xl opacity-40">
                  {project.title}
                </span>
                <span className="font-mono text-[10px] md:text-xs tracking-widest uppercase opacity-40">
                  {project.category}
